...
class Esp8266Info(InfoBase):
    ...
    def __init__(self):
        try:
            ...
            sys.exit(1)
        try:
            self.partitions = self.load_json(f"{CORE_DATA_DIR}/esp8266_partition_schemes.json")
        except Exception as e:  # pylint: disable=broad-exception-caught
            print(f"Error initializing Esp8266Info: {e}")
            sys.exit(1)
    ...
    def _get_partition_scheme(self, scheme_name: str) -> list[dict[str, str]]:
        ...
        if scheme is None:
            print(f"Error: Partition scheme '{scheme_name}' not found in esp8266_partition_schemes.json")
            sys.exit(1)
        return scheme
    ...
    def _get_spiffs_partition(self, scheme: list[dict[str, str]]) -> dict[str, str]:
        ...
        for partition in scheme:
            if partition["name"] == "spiffs":
                return partition
        print(f"Error: SPIFFS partition not found in partition scheme '{scheme}'")
        sys.exit(1)
