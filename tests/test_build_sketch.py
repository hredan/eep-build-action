...
def _setup_tmp_core_data(tmp_path: Any) -> Path:
    ...
    (core_data_dir / "esp32_partition_schemes.json").write_text(json.dumps({}))
...
def test_build_sketch_arduino_cli_extraction_failure(tmp_path: Any, monkeypatch: Any) -> None:
    ...
    with patch("py_modules.helper.download_json_files"), \
        ...
        mock_run.return_value = {"success": False, "stderr": "Extraction failed"}
...
def test_build_sketch_arduino_cli_version_check_fails(tmp_path: Any, monkeypatch: Any) -> None:
    """Test build_sketch handles Arduino CLI version check failure."""
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("INPUT_CORE", "esp32")
    monkeypatch.setenv("INPUT_BOARD", "esp32dev")
    monkeypatch.setenv("INPUT_SKETCH_NAME", "blink")
    ...
    def run_side_effect(cmd: str, **kwargs: Any) -> dict[str, Any]:
        if "version" in cmd:
            return {"success": False, "stderr": "Version check failed"}
        return {"success": True}
